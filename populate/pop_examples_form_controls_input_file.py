...
example_info1.title       = "@input[type='file']@ labelling techniques"
example_info1.permanent_slug = 'file1'
...
* Use a @label@ element with a @for@ attribute that references a @input[type='file']@'s @id@ attribute. 
* Use a @label@ element which encapsulates a @input[type='file']@, and references the input with its file content.
* Have the @input[type='file']@ reference another element on the screen using an @aria-labelledby@ attribute.

h3. Other labeling techniques

The following labeling techniques do NOT affect the text appearing near the file element, which potentially results in the visible label and the label used by assistive technology to be different.
...
* @aria-label@ attribute, labels the file element.
...
example_info1.html        = """
<form>
  <label <HL1>for="<HL2>file1</HL2>"</HL1>>This element labels the file element below using the label for attribute </label>
  <input type="file" id="file1"/>
  <HL1><label><HL2>This element labels the file element below using encapsulation.</HL2> 
    <input type="file" id="file2"/>
  </label></HL1>
  <label id="label3">This element labels the file element below using aria-labelledby.</label>
  <input type="file" <HL1>aria-labelledby="<HL2>label3</HL2>"</HL1> id="file3"/>
  <label id="label4">The element below is referenced by an aria-label attribute and not by this label.</label>
  <input type="file" id="file4" <HL1>aria-label="<HL2>file4</HL2>"</HL1>/>
  <label id="label5">The element below is referenced by an title attribute and not by this label.</label>
  <input type="file" id="file5" <HL1>title="<HL2>file5</HL2>"</HL1>/>
</form>
