...
example_info1.title       = "@input[type='text']@ labelling techniques"
...
* Use a @label@ element with a @for@ attribute that references a @input[type='text']@'s @id@ attribute. 
* Use a @label@ element which encapsulates a @input[type='text']@, and references the input with its text content.
* Have the @input[type='text']@ reference another element on the screen using an @aria-labelledby@ attribute.

h3. Other labeling techniques

The following labeling techniques do NOT affect the text appearing near the text element, which potentially results in the visible label and the label used by assistive technology to be different.
...
* @aria-label@ attribute, labels the text element.
...
example_info1.html        = """
<form>
  <label <HL1>for="<HL2>text1</HL2>"</HL1>>This element labels the text element below using the label for attribute </label>
  <input type="text" id="text1"/>
  <HL1><label><HL2>This element labels the text element below using encapsulation.</HL2> 
    <input type="text" id="text2"/>
  </label></HL1>
  <label id="label3">This element labels the text element below using aria-labelledby.</label>
  <input type="text" <HL1>aria-labelledby="<HL2>label3</HL2>"</HL1> id="text3"/>
  <label id="label4">The element below is referenced by an aria-label attribute and not by this label.</label>
  <input type="text" id="text4" <HL1>aria-label="<HL2>text4</HL2>"</HL1>/>
  <label id="label5">The element below is referenced by an title attribute and not by this label.</label>
  <input type="text" id="text5" <HL1>title="<HL2>text5</HL2>"</HL1>/>
</form>
